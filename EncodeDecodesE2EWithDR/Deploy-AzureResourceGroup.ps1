...
New-AzureRmResourceGroup -Location "$ResourceGroupLocation" -Name $ResourceGroupName -Force
    ...
    $DecodeTemplateFile = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($PSScriptRoot, ("DecodeLogicApp.json")))
    $DecodeTemplateParametersFile = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($PSScriptRoot, ("DecodeLogicApp.parameters.json")))
    $EncodeTemplateFile = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($PSScriptRoot, ("EncodeLogicApp.json")))
    $EncodeTemplateParametersFile = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($PSScriptRoot, ("EncodeLogicApp.parameters.json")))

New-AzureRmResourceGroupDeployment -Name ($ResourceGroupName + '-' + ((Get-Date).ToUniversalTime()).ToString('MMdd-HHmm')) `
									-ResourceGroupName $ResourceGroupName `
									-TemplateFile $DecodeTemplateFile `
									-TemplateParameterFile $DecodeTemplateParametersFile `
									@OptionalParameters `
									-Force -Verbose `
